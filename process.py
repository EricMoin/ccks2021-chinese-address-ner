...
def convert_distance(item, tags):
    ...
    text = item['text']
    spans = [x for x in re.finditer(
        '(0+|(十?[一二三四五六七八九几]+(十|百)?[一二三四五六七八九几]?))米', text)]
    for sp in spans:
        start, end = sp.span()
...
def convert_village(item, tags):
    ...
    text = item['text']
    spans = [x for x in re.finditer(
        '(0+|(十?[一二三四五六七八九])|([一二三四五六七八九]十[一二三四五六七八九]?))[组队社]', text)]
    for sp in spans:
        start, end = sp.span()
...
def _fix_data(ent_tp_cnt, update_files, iob=False):
    ...
    for sentence in data:
        ...
        if iob:
            ...
            tags = iob_iobes(convert_back_to_bio(
                new_entities, [w[0] for w in sentence]))
...
def fix_data():
    ent_tp_cnt = _get_refine_entity(
        [TCDATA_DIR + 'train.conll', TCDATA_DIR + 'dev.conll', TCDATA_DIR + 'extra_train.conll'])
...
def create_extra_train_data():
    ...
    for sentence in data:
        ...
        # 去除噪声开头
        valid_start = ['B-prov', 'B-city', 'B-district',
                       'B-town', 'B-road', 'B-poi', 'B-devzone', 'B-community']
